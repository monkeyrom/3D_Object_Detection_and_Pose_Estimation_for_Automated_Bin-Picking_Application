...
void cmdCallback(const std_msgs::String::ConstPtr& msg)
{
 ...
 /* if (cmd == "home"){
  ...
  if (cmd == "home"){
    char cmd[200];
    sprintf(cmd, "gnome-terminal --tab -e 'rosservice call dobot_bringup/srv/MovJ \"{x: 143.0, y: -580.0, z: 405.0, a: 180.0, b: 0.0, c: 180.0}\"'");
    system(cmd);
  }
  if (cmd == "sleep"){
    char cmd[200];
...
{ 
  ...
  ros::Subscriber sub1_order = n.subscribe("main/target_order", 100, chatterCallback);
  ros::Subscriber sub2 = n.subscribe("Cmd_Talker", 100, cmdCallback);
  ros::Subscriber sub2_order = n.subscribe("main/cmd_talker", 100, cmdCallback);
