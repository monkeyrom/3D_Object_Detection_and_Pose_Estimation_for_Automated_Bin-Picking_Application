...
void cmdCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  if (cmd == "Point1"){
    char cmd[200];
    sprintf(cmd, "gnome-terminal --tab -e 'rosservice call dobot_bringup/srv/MovJ \"{x: 136.0, y: -550.0, z: 260.0, a: 180.0, b: 0.0, c: 180.0}\"'");
    system(cmd);
  }
  if (cmd == "Point1_R"){
    char cmd[200];
  ...
  if (cmd == "Point2"){
    char cmd[200];
    sprintf(cmd, "gnome-terminal --tab -e 'rosservice call dobot_bringup/srv/MovJ \"{x: 20.0, y: -550.0, z: 200.0, a: 180.0, b: 0.0, c: 180.0}\"'");
    system(cmd);
  }
  if (cmd == "Speed100"){
	  char cmd_spdJ[200];
...
{ 
  ...
  ros::Subscriber sub1 = n.subscribe("Object_Position", 100, chatterCallback);
  ros::Subscriber sub2 = n.subscribe("Cmd_Talker", 100, cmdCallback);
...
// Starting
  ...
  char newtmn[200];
  sprintf(newtmn, "gnome-terminal --geometry=90x24+0+0 --window \ --working-directory=/depot --title='Main Control Terminal' --command=\"rosrun CR5_Project main_control\"");
  system(newtmn);
