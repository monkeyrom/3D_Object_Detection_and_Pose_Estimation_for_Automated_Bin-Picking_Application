...
def compute_vector_80(x,y,z,rx,ry,rz):
    # Define the position and orientation of the object
    object_pos = np.array([x, y, z])  # Object position in world frame
    object_ori = np.array([rx, ry, rz])  # Object orientation in world frame (euler angles)
    ...
    p_o_desired = np.array([0, 0, 80])  # 54mm above the object in the object frame
...
def compute_vector_50(x,y,z,rx,ry,rz):
    # Define the position and orientation of the object
    object_pos = np.array([x, y, z])  # Object position in world frame
    object_ori = np.array([rx, ry, rz])  # Object orientation in world frame (euler angles)
    ...
    # Define the desired position of the robot relative to the object frame
    p_o_desired = np.array([0, 0, 54])  # 54mm above the object in the object frame
...
def pick_phase1 (focus_obj):
    if focus_obj == 1 :
        ...
        try :
            ...
            listener.waitForTransform(target_frameid1, source_frameid, rospy.Time(), rospy.Duration(0.1))
            ...
            print('no object_a')
        try :
            listener.waitForTransform(target_frameid2, source_frameid, rospy.Time(), rospy.Duration(0.1))
            ...
            print('no object_b')
        try :
            listener.waitForTransform(target_frameid3, source_frameid, rospy.Time(), rospy.Duration(0.1))
        ...
        try : 
            if multi == 1:
                ...
                y1 = int(v1[1] * 1000)
                z1 = int(v1[2] * 1000)
                rx1 = int(roll1 * 180.0 / math.pi)
                ry1 = int(pitch1 * 180.0 / math.pi)
                rz1 = int(yaw1 * 180.0 / math.pi)

                x2 = int(v2[0] * 1000)
                y2 = int(v2[1] * 1000)
                z2 = int(v2[2] * 1000)
                rx2 = int(roll2 * 180.0 / math.pi)
                ry2 = int(pitch2 * 180.0 / math.pi)
                rz2 = int(yaw2 * 180.0 / math.pi)
                ...
                y3 = int(v3[1] * 1000)
                z3 = int(v3[2] * 1000)
                rx3 = int(roll3 * 180.0 / math.pi)
                ry3 = int(pitch3 * 180.0 / math.pi)
                rz3 = int(yaw3 * 180.0 / math.pi)
                ...
                y4 = int(v4[1] * 1000)
                z4 = int(v4[2] * 1000)
                rx4 = int(roll4 * 180.0 / math.pi)
                ry4 = int(pitch4 * 180.0 / math.pi)
                rz4 = int(yaw4 * 180.0 / math.pi)
            ...
            robo_compute = compute_vector_80(x_cal,y_cal,z_cal,rx_cal,ry_cal,rz_cal)
            target = ObjectMsg()
...
def pick_phase2 (x,y,z,rx,ry,rz):
    ...
    robo_compute = compute_vector_50(x,y,z,rx,ry,rz)
    target = ObjectMsg()
    target.x = robo_compute[0]
    target.y = robo_compute[1]
    target.z = robo_compute[2]
...
def start_order():
    ...
    while True :
        ...
        if step == 0 and home_check == -1 :         # Check Robot and not found
            ...
            sleep(1.5)                              # sleep 1500 milisec swiching algorithm
            ...
            if obj_target1 == -1:                       # Object lost
                ...
                robo_compute1 = compute_vector_80(x,y,z,rx,ry,rz)
                x = obj_target1[0]
                y = obj_target1[1]
                z = obj_target1[2]
                rx = obj_target1[3]
                ry = obj_target1[4]
                rz = obj_target1[5]
                pose_check = robot_check_pose(robo_compute1[0],robo_compute1[1],robo_compute1[2]+30)    # Check Robot position (25 mm above object)
                sleep(0.1)
                if pose_check == 1 :
                    pick_phase2(focus_obj)                                                              # Robot at the position (25 mm above object)
                    robo_compute2 = compute_vector_50(x,y,z,rx,ry,rz)                                   # move to object
                    sunction_cup(1)                                                                     # Set sunction cup on
                    pose_check = robot_check_pose(robo_compute2[0],robo_compute2[1],robo_compute2[2])   # Check Robot position (at object)
                    sleep(0.1)
                    print ("robot picking object")
                    if pose_check == 1 :
                        print ('Picked object')
                        pick_phase3(x,y,z,rx,ry,rz)
                        pose_check = robot_check_pose(robo_compute2[0],robo_compute2[1],robo_compute2[2]+100)
                        sleep(0.1)
                        step = 3
                    if pose_check == -1 :
                        break
